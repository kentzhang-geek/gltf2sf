...
std::unique_ptr<sf::CameraT> ParseCamera(const tinygltf::Camera &glcam, const tinygltf::Node &glnode, const glm::mat4 & parent_transform) {
    std::unique_ptr<sf::CameraT> sfcam = std::make_unique<sf::CameraT>();
    ...
    if (glcam.type == "perspective") {
        ...
        eye = eye / eye.w;
        sfcam->eye = std::make_unique<sf::Vec3d>(GLVecToSFVec(glm::vec3(eye)));
        glm::vec4 center(0.0f, 0.0f, -1.0f, 1.0f);
        center = parent_transform * center;
        center = center / center.w;
        sfcam->center = std::make_unique<sf::Vec3d>(GLVecToSFVec(glm::vec3(center)));
        glm::vec4 up(0.0f, 1.0f, 0.0f, 1.0f);
        up = parent_transform * up;
        up = up / up.w;
        sfcam->upAxis = std::make_unique<sf::Vec3d>(GLVecToSFVec(glm::vec3(up)));
    }
    else {
